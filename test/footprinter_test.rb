  ...
  describe 'ユーザーのアクセス履歴を' do
    ...
    describe '対象のモデル毎に' do
      it '取得できること' do
        ...
        expect(user_1.access_histories_for(Footprintable).map(&:footprintable).pluck(:name)).must_equal 5.downto(1).map { |i| "footprintable#{i}" }
      end

      it '件数を絞り込めること' do
        expect(user_1.access_histories_for(Footprintable, 3).count).must_equal 3
        expect(user_1.access_histories_for(Footprintable, 3).map(&:footprintable).pluck(:name)).must_equal 5.downto(3).map { |i| "footprintable#{i}" }
      end
    end
    ...
    describe '全てのモデルを通じて' do
      it '取得できること' do
        ...
        expect(user_1.access_histories.map(&:footprintable).pluck(:name)).must_equal footprintable_names
      end
